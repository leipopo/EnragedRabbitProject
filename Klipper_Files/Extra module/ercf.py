...
class Ercf:
    ...
    def __init__(self, config):
        ...
        self.end_of_bowden_to_nozzle = config.getfloat(
            "end_of_bowden_to_nozzle", above=30.0
        )
        self.preload_length_at_end_of_bowden = config.getfloat(
            "preload_length_at_end_of_bowden", 10
        )
        self.num_moves = config.getint("num_moves", 2)
        self.parking_distance = config.getfloat(
            "parking_distance", 23.0, above=15.0, below=30.0
        )
        self.encoder_move_step_size = config.getfloat(
            "encoder_move_step_size", 15.0, above=5.0, below=25.0
        )
        ...
        self.min_temp_extruder = config.getfloat("min_temp_extruder", 180.0)
        self.calibration_bowden_length = config.getfloat("calibration_bowden_length")
        self.unload_buffer = config.getfloat("unload_buffer", 30.0)
        self.sync_unload_length = config.getfloat("sync_unload_length", 20.0)
        self.unload_parking_position = config.getfloat("unload_parking_position", 0.0)
        self.extruder_homing_max = config.getfloat("extruder_homing_max", 50.0)
        self.extruder_homing_step = config.getfloat("extruder_homing_step", 2.0)
        ...
        if self.enable_endless_spool == 1 and len(self.endless_spool_groups) != len(
        ...
        self.gcode.register_command(
            "ERCF_SERVO_UP", self.cmd_ERCF_SERVO_UP, desc=self.cmd_ERCF_SERVO_UP_help
        )
        self.gcode.register_command(
            "ERCF_TEST_SERVO",
        ...
        self.gcode.register_command(
            "ERCF_CHANGE_TOOL_STANDALONE",
            self.cmd_ERCF_CHANGE_TOOL_STANDALONE,
            desc=self.cmd_ERCF_CHANGE_TOOL_STANDALONE_help,
        )
        self.gcode.register_command(
            "ERCF_HOME", self.cmd_ERCF_HOME, desc=self.cmd_ERCF_HOME_help
        )
        self.gcode.register_command(
            "ERCF_PAUSE", self.cmd_ERCF_PAUSE, desc=self.cmd_ERCF_PAUSE_help
        )
        self.gcode.register_command(
            "ERCF_UNLOCK", self.cmd_ERCF_UNLOCK, desc=self.cmd_ERCF_HOME_help
        )
        self.gcode.register_command(
            "ERCF_EJECT", self.cmd_ERCF_EJECT, desc=self.cmd_ERCF_EJECT_help
        )
        self.gcode.register_command(
            "ERCF_RESET_STATS",
        ...
        self.gcode.register_command(
            "ERCF_CALIBRATE", self.cmd_ERCF_CALIBRATE, desc=self.cmd_ERCF_CALIBRATE_help
        )
        self.gcode.register_command(
            "ERCF_CALIBRATE_SINGLE",
        ...
        self.gcode.register_command(
            "ERCF_DISPLAY_ENCODER_POS",
            self.cmd_ERCF_DISPLAY_ENCODER_POS,
            desc=self.cmd_ERCF_DISPLAY_ENCODER_POS_help,
        )
        self.gcode.register_command(
            "ERCF_TEST_GRIP", self.cmd_ERCF_TEST_GRIP, desc=self.cmd_ERCF_TEST_GRIP_help
        )
        self.gcode.register_command(
            "ERCF_TEST_MOVE_GEAR",
    ...
    def handle_connect(self):
        ...
        self.encoder_sensor = self.printer.lookup_object(
            "filament_motion_sensor encoder_sensor"
        )
        self.printer.register_event_handler(
            "klippy:ready", self._setup_heater_off_reactor
        )
    ...
    def _dump_statistics(self):
        if self.log_statistics:
            ...
            self._log_info(
                "%s spent loading"
                % self._seconds_to_human_string(self.time_spent_loading)
            )
            self._log_info(
                "%s spent unloading"
                % self._seconds_to_human_string(self.time_spent_unloading)
            )
            self._log_info(
                "%s spent paused (%d pauses total)"
                % (
                    self._seconds_to_human_string(self.time_spent_paused),
                    self.total_pauses,
                )
            )
    ...
    def _servo_set_angle(self, angle):
        ...
        self.gcode.run_script_from_command(
            "SET_SERVO SERVO=ercf_servo ANGLE=%1.f" % angle
        )
    ...
    def _servo_down(self):
        ...
        self.gear_stepper.do_move(0.0, 25, self.gear_stepper_accel, sync=0)
        self.toolhead.dwell(0.1)
        self.gear_stepper.do_move(0.5, 25, self.gear_stepper_accel, sync=0)
        self.toolhead.dwell(0.1 + self.extra_servo_dwell_down / 1000.0)
        self.gear_stepper.do_move(0.0, 25, self.gear_stepper_accel, sync=0)
    ...
    def _servo_up(self):
        ...
        self.toolhead.dwell(0.25 + self.extra_servo_dwell_up / 1000.0)
        self._servo_off()
    ...
    def _do_calculate_calibration_ref(
        self, extruder_homing_length=400, extruder_homing_step=2
    ):
        ...
        self._log_debug(
            "Moved to calibration distance %.1f - encoder reads %.1f"
            % (self.calibration_bowden_length, self._counter.get_distance())
        )

        final_position = self._home_to_extruder(
            extruder_homing_length, extruder_homing_step
        )
        if self.is_paused:
        ...
        self.gcode.run_script_from_command(
            "SAVE_VARIABLE VARIABLE=ercf_calib_ref VALUE=%.1f" % final_position
        )
        self.gcode.run_script_from_command(
            "SAVE_VARIABLE VARIABLE=ercf_calib_0 VALUE=1.0"
        )
        self.gcode.run_script_from_command(
            "SAVE_VARIABLE VARIABLE=ercf_calib_version VALUE=2"
        )
    ...
    def _do_calculate_calibration_ratio(self, tool):
        ...
        load_length = self.calibration_bowden_length - 100.0
        ...
        self._log_info(
            "Calibration move to %.1f read distance %.1f - Ratio is %.12f"
            % (load_length, final_position, ratio)
        )
        self.gcode.run_script_from_command(
            "SAVE_VARIABLE VARIABLE=ercf_calib_%d VALUE=%.12f" % (tool, ratio)
        )
    ...
    cmd_ERCF_CALIB_SELECTOR_help = (
        "Calibration of the selector position for a defined Tool"
    )
    ...
    def cmd_ERCF_CALIB_SELECTOR(self, gcmd):
        ...
        self.selector_stepper.do_homing_move(
            -move_length, 50, self.selector_stepper.accel, True, True
        )
    ...
    def cmd_ERCF_TEST_MOVE_GEAR(self, gcmd):
        length = gcmd.get_float("LENGTH", 200.0)
        speed = gcmd.get_float("SPEED", 50.0)
        accel = gcmd.get_float("ACCEL", 200.0)
        self.gear_stepper.do_set_position(0.0)
    ...
    def cmd_ERCF_TEST_LOAD_SEQUENCE(self, gcmd):
        ...
        for l in range(loops):
            ...
            for t in range(len(self.selector_offsets)):
                ...
                self._log_info(
                    "testing tool %d / %d" % (tool, len(self.selector_offsets))
                )
                self._select_tool(tool)
    ...
    def _setup_heater_off_reactor(self):
        self.reactor = self.printer.get_reactor()
        self.heater_off_handler = self.reactor.register_timer(
            self._handle_pause_timeout, self.reactor.NEVER
        )
    ...
    def _pause(self):
        ...
        self.paused_extruder_temp = self.printer.lookup_object(
            "extruder"
        ).heater.target_temp
        self._servo_up()
        self.is_paused = True
        self.gcode.run_script_from_command(
            "SET_IDLE_TIMEOUT TIMEOUT=%d" % self.timeout_pause
        )
        self.reactor.update_timer(
            self.heater_off_handler, self.reactor.monotonic() + self.disable_heater
        )
        self._log_info(
            "An issue with the ERCF has been detected and the ERCF has been PAUSED"
        )
        self._log_info(
            'When you intervene to fix the issue, first call the "ERCF_UNLOCK" Gcode'
        )
    ...
    def _set_above_min_temp(self):
        if not self.printer.lookup_object("extruder").heater.can_extrude:
            self._log_info(
                "M118 Heating extruder above min extrusion temp (%.1f)"
                % self.min_temp_extruder
            )
    ...
    cmd_ERCF_SET_END_OF_BOWDEN_TO_NOZZLE_help = (
        "Update the end_of_bowden_to_nozzle value"
    )

    def cmd_ERCF_SET_END_OF_BOWDEN_TO_NOZZLE(self, gcmd):
        val = gcmd.get_float("VALUE", minval=30.0)
    ...
    def _check_filament_in_extruder(self, move_size=20, threshold=1):
        ...
        self._log_trace(
            "Extruder Moved %.2f, Read %.2f, Threshold %.2f"
            % (move_size, final_encoder_pos, threshold)
        )
    ...
    def _check_filament_in_encoder(self):
        ...
        final_encoder_pos = self._counter.get_distance()
        self._log_trace(
            "After buzzing gear motor, encoder read %.1f" % final_encoder_pos
        )
        self._servo_up()
        return final_encoder_pos > 0.0
    ...
    def _unload_from_extruder(self):
        ...
        for i in range(3):
            ...
            self._log_trace(
                "Test extruder move of %1.fmm recorded %1.fmm at the extruder"
                % (self.encoder_move_step_size, moved_length)
            )
            if moved_length <= 1.0:
    ...
    def _unload_encoder(self, max_steps=5):
        ...
        for step in range(max_steps):
            ...
            self._log_trace(
                "Unloading from encoder - step %d.  Moved %.1f, encoder read %.1f (delta %.1f)"
                % (step, self.encoder_move_step_size, dist_moved, delta)
            )
            if delta >= 3.0:
    ...
    def _unload_from_end_of_bowden(self, length):
        ...
        self._log_info(
            "Unload move done: %.1f / %.1f (diff: %.1f)"
        ...
        if delta_length >= 3.0:
            ...
            delta_length = length - counter_distance
            self._log_info(
                "Correction unload move done: %.1f / %.1f (diff: %.1f)"
                % (counter_distance, length, delta_length)
            )
        ...
        if delta_length > 15.0:
            self._log_info(
                "Too much slippage detected during the unload: %.1f / %.1f (diff: %.1f)"
                % (counter_distance, length, delta_length)
            )
            self._pause()
            return
    ...
    def _load_into_encoder(self):
        ...
        self._log_trace(
            "Initial load into encoder of %.1fmm - read %.1f on encoder"
            % (self.LONG_MOVE_THRESHOLD, self._counter.get_distance())
        )
        ...
        self._gear_stepper_move_wait(self.LONG_MOVE_THRESHOLD)
        self._log_trace(
            "Retry load into encoder of %.1fmm - read %.1f on encoder"
            % (self.LONG_MOVE_THRESHOLD, self._counter.get_distance())
        )
        if self._counter.get_distance() <= 6.0:
            self._log_info(
                "Error loading filament - not enough detected at encoder after retry"
            )
            self._pause()
            return 0
        self.loaded_status = self.LOADED_STATUS_PARTIAL
    ...
    def _load_to_end_of_bowden(self, length):
        ...
        for i in range(2):
            if delta_length >= 6.0:
                ...
                delta_length = length - counter_distance
                self._log_info(
                    "Correction load move done: %.1f / %.1f (diff: %.1f)"
                    % (counter_distance, length, delta_length)
                )
            else:
                break
        ...
        if delta_length > self.LONG_MOVE_THRESHOLD:
            self._log_info(
                "Too much slippage detected during the load: %.1f / %.1f (diff: %.1f)"
                % (counter_distance, length, delta_length)
            )
            self._pause()
            return
    ...
    def _home_to_extruder(self, length, step):
        ...
        for i in range(int(length / step)):
            ...
            self._log_trace(
                "Step #%d: pos: %.1f, last delta: %.1f"
                % (i, post_move_position, post_move_position - pre_move_position)
            )
            if post_move_position - pre_move_position < step / 2.0:
    ...
    def _load_to_nozzle(self):
        ...
        for i in range(int((self.preload_length_at_end_of_bowden - 5) / step_len)):
            self.gear_stepper.do_set_position(0.0)
            self.gear_stepper.do_move(step_len, 10, self.gear_stepper_accel)
            pos[3] += step_len
        ...
        #     self.end_of_bowden_to_nozzle-self.preload_length_at_end_of_bowden,
        #     30,
        ...
        moved_length = self._counter.get_distance()
        self._log_debug(
            "Extruder move to nozzle attempted %.1f - encoder read %.1f"
            % (self.end_of_bowden_to_nozzle, moved_length)
        )
    ...
    def _do_home_selector(self, tool=0):
        ...
        selector_length = 20.0 + num_channels * 21.0 + (num_channels / 3) * 5.0
        self._log_debug(
            "Moving up to %.1fmm to home a %d channel ERCF"
            % (selector_length, num_channels)
        )
        ...
        if self.sensorless_selector == 1:
            self.selector_stepper.do_set_position(0.0)
            self.selector_stepper.do_homing_move(
                -selector_length, 60, self.selector_stepper.accel, True, True
            )
            self.selector_stepper.do_set_position(0.0)
        else:
            self.selector_stepper.do_set_position(0.0)
            self.selector_stepper.do_homing_move(
                -selector_length, 100, self.selector_stepper.accel, True, True
            )
            self.selector_stepper.do_set_position(0.0)
            self.selector_stepper.do_move(5.0, 100, 0)
            self.selector_stepper.do_homing_move(
                -10, 10, self.selector_stepper.accel, True, True
            )
            self.selector_stepper.do_set_position(0.0)
        ...
        self.selector_stepper.do_move(
            self.selector_offsets[tool], 50, self.selector_stepper.accel
        )
        self.toolhead.wait_moves()
        endstop_lookup = "manual_stepper selector_stepper"
        if self.sensorless_selector:
            endstop_lookup = "manual_stepper gear_stepper"
        selector_state = self.printer.lookup_object("query_endstops").get_status(0)[
            "last_query"
        ][endstop_lookup]
        if selector_state != 1:
    ...
    def _do_home(self, tool=0):
        if self._get_calibration_version() != 2:
            self._log_info(
                "You are running an old calibration version.  It is strongly recommended that you rerun 'ERCF_CALIBRATE_SINGLE TOOL=0' to generate an updated calibration value"
            )
        self._disable_encoder_sensor()
        self.is_homed = True
        if self.is_paused:
    ...
    def cmd_ERCF_CHANGE_TOOL(self, gcmd):
        tool = gcmd.get_int("TOOL")
        initial_tool_string = (
            "unknown tool" if self.tool_selected else ("T%d" % self.tool_selected)
        )
        self._log_info(
            "Tool swap requested, from %s to T%d" % (initial_tool_string, tool)
        )
        self.gcode.run_script_from_command(
            "M117 %s -> T%d" % (initial_tool_string, tool)
        )
    ...
    def cmd_ERCF_ENCODER_RUNOUT(self, gcmd):
        ...
        if moved > 0.0:
            ...
            if self.enable_endless_spool:
                initial_pa = self.printer.lookup_object("extruder").get_status(0)[
                    "pressure_advance"
                ]
                ...
                while next_check != self.tool_selected:
                    if next_check > len(self.selector_offsets):
                        next_check = 0
                    self._log_trace(
                        "Checking tool %d to see if it matches the right group"
                        % next_check
                    )
                    if (
                        self.endless_spool_groups[next_check]
                        == self.endless_spool_groups[self.tool_selected]
                    ):
                ...
                if next_tool == -1:
                    self._log_info(
                        "No more spools found in group %d - manual intervention is required"
                        % self.endless_spool_groups[self.tool_selected]
                    )
                    return
                self.gcode.run_script_from_command(
                ...
                self.gcode.run_script_from_command(
                    "SET_PRESSURE_ADVANCE ADVANCE=%.10f" % initial_pa
                )
